...
<h2 class="">Nuevo DevolucionesCompraDetalles</h2>
</div>
...
@endif
<form class= "form-horizontal" action="{{ route('DevolucionesCompraDetalles.create') }}" method="POST" method="POST" enctype="multipart/form-data">
@csrf
<div class="card-body">
<div class="form-group">
<label for="devoluciones_compra_id" class="col-sm-4 control-label">devoluciones_compra_id</label>
<input type="text" maxlength="20" name="devoluciones_compra_id" id="devoluciones_compra_id"  class="form-control" placeholder="devoluciones_compra_id">
...
<SELECT  name="material_id" id="material_id" class="form-control">
 ...
 <option value="{{ $row->id }}" {{ ($row->id== $DevolucionesCompraDetalles->material_id?'selected':'') }}>{{ $row->nombre_material }}</option>
 @endforeach
</SELECT>
